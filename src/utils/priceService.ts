...
const fetchWithTimeout = async (url: string, timeout = 5000, headers?: Record<string, string>) => {
        ...
        for (const url of yahooUrls) {
            try {
                ...
                if (response.ok) {
                    ...
                    if (chartPrice) return parseFloat(chartPrice);
                    // v7/quote format
                    const quotePrice = data.quoteResponse?.result?.[0]?.regularMarketPrice;
                    if (quotePrice) return parseFloat(quotePrice);
                }
            } catch { /* try next URL */ }
        }
    } catch (e) {
...
export const getCryptoPrice = async (symbol: string): Promise<number> => {
    ...
    if (COINGECKO_API_KEY && isKnownCrypto) {
        try {
            ...
            const response = await fetchWithTimeout(
                `https://pro-api.coingecko.com/api/v3/simple/price?ids=${geckoId}&vs_currencies=${vsCurrency}`,
                5000,
    ...
    // 5. Try CoinCap
    try {
        const idMap: Record<string, string> = { 'BTC': 'bitcoin', 'ETH': 'ethereum', 'USDT': 'tether', 'SOL': 'solana', 'BNB': 'binance-coin' };
    ...
    if (COINGECKO_API_KEY) {
        ...
        if (cryptoSymbols.length > 0) {
            try {
                ...
                const response = await fetchWithTimeout(
                    `https://pro-api.coingecko.com/api/v3/simple/price?ids=${uniqueIds}&vs_currencies=usd&include_24hr_change=true&include_24hr_vol=true&include_market_cap=true`,
                    8000,
                    { 'x-cg-pro-api-key': COINGECKO_API_KEY }
                );
