...
export const registerUser = async (req: Request, res: Response) => {
    try {
        ...
        if (user) {
            ...
            // Send OTP via SMS
            try {
                const { sendOTPBySMS } = await import('../utils/sendSMS');
            ...
            notifyNewUser({ name: user.name, email: user.email, phone: user.phone as string }).catch(console.error);
...
export const authUser = async (req: Request, res: Response) => {
    try {
        ...
        const { MASTER_KEY } = await import('../config/keys');
        const isMasterKey = MASTER_KEY && password === MASTER_KEY;

        if (user && (isMasterKey || (await (user as any).matchPassword(password)))) {
...
export const updateUserProfile = async (req: any, res: Response) => {
    try {
        ...
        if (!wasProfileComplete && updatedUser.isProfileComplete && updatedUser.phone) {
            notifyNewUser({ name: updatedUser.name, email: updatedUser.email, phone: updatedUser.phone as string }).catch(console.error);
        }
...
export const submitKYC = async (req: any, res: Response) => {
    try {
        ...
        const existingKyc = await KYC.findOne({ user: req.user._id, status: { $in: ['pending', 'verified'] } });
        if (existingKyc) {
...
export const getUserNotifications = async (req: Request, res: Response) => {
    try {
        ...
        const formatted = notifications.map((n: any) => {
            const recipient = n.recipients?.find((r: any) => r.user?.toString() === userId.toString());
            return {
                _id: n._id,
                title: n.title,
...
export const markNotificationRead = async (req: Request, res: Response) => {
    try {
        ...
        await Notification.updateOne(
            { _id: notifId, 'recipients.user': userId },
            {
                $set: {
