...
import { getCryptoPrice, getMultiplePrices, getDetailedMarketData, getStockPrice } from '../utils/priceService';
...
export const getMarketPrices = async (req: Request, res: Response) => {
    try {
        const symbols = (req.query.symbols as string)?.split(',') || ['BTC/USDT', 'ETH/USDT', 'SOL/USDT', 'BNB/USDT', 'XRP/USDT'];
...
// @access  Public
export const getDetailedMarketPrices = async (req: Request, res: Response) => {
    try {
        ...
        const data = await getDetailedMarketData(symbols);
        res.json(data);
    } catch (error) {
...
// @access  Private
export const openTrade = async (req: any, res: Response) => {
    try {
        ...
        const priceForMargin = isLimit
            ? Number(limitPrice)
            : (clientPrice && Number(clientPrice) > 0 ? Number(clientPrice) : await getCryptoPrice(asset));
        ...
        if (type === 'futures') {
            ...
            if (isBuyOrder) {
                ...
                const openBuys = await Trade.find({
...
// @access  Private
export const getMyOpenTrades = async (req: any, res: Response) => {
    try {
        ...
        const assetMarketMap: Record<string, string> = {};
        for (const trade of trades) {
            if (!assetMarketMap[trade.asset]) assetMarketMap[trade.asset] = trade.market || 'crypto';
        }
        ...
        const liquidatedNow: Set<string> = new Set();
        for (const trade of trades) {
            if (trade.status !== 'open' || trade.type !== 'futures' || trade.leverage <= 1) continue;
...
// @access  Private
export const closeTrade = async (req: any, res: Response) => {
    try {
        ...
        try {
            closingPrice = await getCryptoPrice(trade.asset);
        } catch {
...
export const adminModifyEntryPrice = async (req: any, res: Response) => {
    try {
        ...
        if (trade.type === 'futures' && trade.leverage > 1) {
            ...
            trade.liquidationPrice = parseFloat(
                (Number(entryPrice) * (1 - (direction / trade.leverage))).toFixed(2)
            );
        }
        ...
        let liveCurrentPrice: number | null = null;
        let newUnrealizedPnL: number | null = null;
        let newPnlPercentage: number | null = null;
        try {
            liveCurrentPrice = await getCryptoPrice(trade.asset);
            const direction = (trade.side === 'buy' || trade.side === 'long') ? 1 : -1;
            newUnrealizedPnL = parseFloat(
                (direction === 1
                    ? (liveCurrentPrice - trade.entryPrice) * trade.amount
                    : (trade.entryPrice - liveCurrentPrice) * trade.amount
                ).toFixed(2)
            );
...
// @access  Private (Admin Only)
export const adminGetAllTrades = async (req: Request, res: Response) => {
    try {
...
// @access  Private (Admin Only)
export const adminOpenTradeForUser = async (req: Request, res: Response) => {
    try {
        ...
        let price: number;
        try {
            price = entryPrice ? Number(entryPrice) : await getCryptoPrice(asset);
        } catch (priceError) {
            return res.status(400).json({ 
        ...
        const populatedTrade = await Trade.findById(trade._id).populate('user', 'name email');
...
// @access  Private (Admin Only)
export const adminCloseTrade = async (req: Request, res: Response) => {
    try {
        const trade = await Trade.findById(req.params.id);
        if (!trade || trade.status !== 'open') {
            return res.status(404).json({ message: 'Open trade not found' });
        ...
        const closingPrice = req.body.closePrice ? Number(req.body.closePrice) : await getCryptoPrice(trade.asset);
...
// @access  Private (Admin Only)
export const adminGetLiquidationStats = async (req: Request, res: Response) => {
    try {
        const totalLiquidated = await Trade.countDocuments({ status: 'liquidated' });
        const todayStart = new Date();
        todayStart.setHours(0, 0, 0, 0);
        const liquidatedToday = await Trade.countDocuments({ status: 'liquidated', updatedAt: { $gte: todayStart } });

        const last7Days = new Date();
        last7Days.setDate(last7Days.getDate() - 7);
        const liquidatedWeek = await Trade.countDocuments({ status: 'liquidated', updatedAt: { $gte: last7Days } });

        // Total margin lost to liquidations
        const marginStats = await Trade.aggregate([
        ...
        const openFutures = await Trade.find({ status: 'open', type: 'futures', leverage: { $gt: 1 } });
        const uniqueAssets = [...new Set(openFutures.map(t => t.asset))];
        const livePrices: Record<string, number> = {};
        await Promise.all(uniqueAssets.map(async (asset) => {
            try { livePrices[asset] = await getCryptoPrice(asset); } catch { livePrices[asset] = 0; }
        }));
        ...
        for (const trade of openFutures) {
            ...
            if (marginRatio < 0.3) atRiskCount++;
        }
...
// @access  Private (Admin Only)
export const adminGetLiquidations = async (req: Request, res: Response) => {
    try {
        const { page = 1, limit = 50, asset, liquidatedBy, search } = req.query;

        const filter: any = { status: 'liquidated' };
        if (asset) filter.asset = asset;
        if (liquidatedBy) filter.liquidatedBy = liquidatedBy;
...
// @access  Private (Admin Only)
export const adminGetAtRiskTrades = async (req: Request, res: Response) => {
    try {
        const openFutures = await Trade.find({ status: 'open', type: 'futures', leverage: { $gt: 1 } })
            .populate('user', 'name email')
            .sort({ createdAt: -1 });
...
// @access  Private (Admin Only)
export const adminManualLiquidate = async (req: any, res: Response) => {
    try {
        const trade = await Trade.findById(req.params.id).populate('user', 'name email');
        if (!trade || trade.status !== 'open') {
            return res.status(404).json({ message: 'Open trade not found' });
        ...
        const currentPrice = await getCryptoPrice(trade.asset);
        ...
        await createAuditLog({
            ...
            performedBy: req.admin?._id?.toString() || 'unknown',
            performedByRole: 'admin',
            targetUser: (trade.user as any)?._id?.toString() || trade.user?.toString(),
            targetResource: trade._id?.toString(),
            details: {
                asset: trade.asset,
                side: trade.side,
...
// @access  Private (Admin Only)
export const adminAdjustLiquidationPrice = async (req: any, res: Response) => {
    try {
...
export const runLiquidations = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (allTrades.length === 0) return res.json({ liquidated: 0, filled: 0 });
            ...
            if ((trade as any).status === 'pending' && (trade as any).orderType === 'limit') {
                ...
                if (shouldFill) {
                    ...
                    if (trade.type === 'futures' && trade.leverage > 1) {
                        ...
                        (trade as any).liquidationPrice = parseFloat((limitPrice * (1 - (dir / trade.leverage))).toFixed(2));
                    }
                    await trade.save();
                    filled++;
                    await Notification.create({
...
// @access  Private (Admin Only)
export const adminCleanupStaleSells = async (req: any, res: Response) => {
    try {
        const result = await Trade.updateMany(
            { type: 'spot', side: { $in: ['sell', 'short'] }, status: 'open' },
            { $set: { status: 'closed', closedAt: new Date(), pnl: 0 } }
        );
