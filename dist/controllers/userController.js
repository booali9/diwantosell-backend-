...
async function createLoginSession(userId, req) {
    ...
    const token = (0, generateToken_1.default)(userId, sessionId);
    return { token, sessionId };
}
/**
 * Check if the IP is a known VPN, proxy, or hosting node.
 */
async function checkVpnOrProxy(ipAddress) {
...
const getUserProfile = async (req, res) => {
    try {
        ...
        const fullUser = await User_1.default.findById(req.user._id).select('fundPassword');
        const userObj = user.toObject();
        userObj.hasFundPassword = !!fullUser?.fundPassword;
        res.json(userObj);
    }
    catch (error) {
        res.status(500).json({ message: 'Server error', error: error instanceof Error ? error.message : 'Unknown error' });
...
const enable2FA = async (req, res) => {
    try {
        ...
        if (!(0, totp_1.verifyTOTP)(code, secret)) {
...
const disable2FA = async (req, res) => {
    try {
        ...
        if (!user)
            return res.status(404).json({ message: 'User not found' });
        if (!user.isGoogleAuthenticatorEnabled || !user.googleAuthenticatorSecret) {
        ...
        if (!(0, totp_1.verifyTOTP)(code, user.googleAuthenticatorSecret)) {
...
const getDeviceSessions = async (req, res) => {
    try {
        const sessions = await DeviceSession_1.default.find({
            user: req.user._id,
            isActive: true,
        }).sort({ lastActive: -1 });
