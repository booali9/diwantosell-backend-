...
exports.transferFunds = exports.recordInternalTransfer = exports.withdrawFunds = exports.sendWithdrawalOTP = exports.getWithdrawalFee = exports.notifyDeposit = exports.getTransactions = exports.simulateDeposit = exports.directDeposit = exports.getDepositStatus = exports.nowpaymentsWebhook = exports.createDeposit = exports.getAddress = exports.getBalance = void 0;
...
const getAddress = async (req, res) => {
    try {
        ...
        const requestedAsset = (asset || 'USDT').toUpperCase();
...
const createDeposit = async (req, res) => {
    try {
        ...
        (0, telegramService_1.notifyDepositRequest)({ email: user.email, name: user.name }, Number(amount), `${asset} on ${network || 'Tron'} (NowPayments)`).catch(console.error);
        // Determine the URLs
        ...
        const payment = await (0, nowpaymentsService_1.createNowPayment)({
            amount: Number(amount),
...
const nowpaymentsWebhook = async (req, res) => {
    try {
        ...
        if (!isSandbox) {
            ...
            if (!signature || !(0, nowpaymentsService_1.verifyNowPaymentsIPN)(body, signature)) {
        ...
        const transaction = await Transaction_1.default.findById(order_id);
        if (!transaction) {
...
// @access  Private
const directDeposit = async (req, res) => {
    try {
        ...
        // Telegram Notification
        (0, telegramService_1.notifyDepositRequest)({ email: user.email, name: user.name }, num, `${(asset || 'USDT').toUpperCase()} on ${network}`).catch(console.error);
        res.json({
...
const simulateDeposit = async (req, res) => {
    try {
        ...
        const transaction = await Transaction_1.default.create({
            ...
            amount: Number(amount),
            status: 'completed',
            txHash: 'SIM_' + crypto_1.default.randomBytes(16).toString('hex'),
            network: 'Simulated',
...
const getTransactions = async (req, res) => {
    try {
        const transactions = await Transaction_1.default.find({ user: req.user._id, isVisible: true }).sort({ createdAt: -1 });
...
// @access  Private
const notifyDeposit = async (req, res) => {
    try {
        ...
        if (depositAsset === 'BTC')
            walletAddress = MASTER_WALLETS.BTC;
        else if (depositNetwork.toLowerCase().includes('trc20') || depositNetwork.toLowerCase().includes('tron'))
            walletAddress = MASTER_WALLETS.USDT_TRC20;
        else
            walletAddress = MASTER_WALLETS.USDT_EVM;
        ...
        // Telegram Notification
        (0, telegramService_1.notifyDepositRequest)({ email: user.email, name: user.name }, Number(amount), `${depositAsset} on ${depositNetwork}`).catch(console.error);
        res.json({
...
exports.notifyDeposit = notifyDeposit;
// @desc    Get configured withdrawal fee
// @route   GET /api/wallet/fee
// @access  Private
const getWithdrawalFee = async (req, res) => {
    try {
...
const sendWithdrawalOTP = async (req, res) => {
    try {
        ...
        await user.save();
        try {
            await (0, sendEmail_1.sendOTPEmail)(user.email, otp);
...
const withdrawFunds = async (req, res) => {
    try {
        ...
        if (user.isGoogleAuthenticatorEnabled) {
            ...
            const { verifyTOTP } = await Promise.resolve().then(() => __importStar(require('../utils/totp')));
            if (!verifyTOTP(google2faCode, user.googleAuthenticatorSecret)) {
        ...
        // Telegram Notification
        (0, telegramService_1.notifyWithdrawRequest)({ email: user.email, name: user.name }, Number(amount), asset || 'USDT', address).catch(console.error);
        res.json({
...
// @access  Private
const recordInternalTransfer = async (req, res) => {
    try {
        ...
        const from = (fromAccount || '').toLowerCase();
        const to = (toAccount || '').toLowerCase();
...
exports.recordInternalTransfer = recordInternalTransfer;
...
const transferFunds = async (req, res) => {
    try {
        ...
        const sender = await User_1.default.findById(req.user._id);
        if (!sender) {
        ...
        const isFundMatch = await sender.verifyFundPassword(fundPassword);
        if (!isFundMatch) {
            return res.status(401).json({ message: 'Incorrect fund password' });
        }
        // 3. Verify Google 2FA if enabled
        if (sender.isGoogleAuthenticatorEnabled) {
            ...
            const { verifyTOTP } = await Promise.resolve().then(() => __importStar(require('../utils/totp')));
            if (!verifyTOTP(google2faCode, sender.googleAuthenticatorSecret)) {
                return res.status(400).json({ message: 'Invalid Google Authenticator code' });
            }
        }
        // 4. Verify Email OTP
        ...
        const senderDefaultNetwork = sender.defaultNetwork || 'Internal Ledger';
